...
# -*- coding: utf-8 -*-
"""
Al-Jarida Multi-Bank Analysis Script with Vision API Integration
Adapted to use Vision API methodology while preserving Al-Jarida download process
"""
...
# Configuration - Keep Al-Jarida download directory structure
BASE_DATA_DIR = os.getenv("DATA_DIR", "/tmp/bank_news_data")
download_dir = os.path.join(BASE_DATA_DIR, "AlJarida")
...
async def download_aljarida():
    """Download the Al-Jarida newspaper PDF - PRESERVE ORIGINAL METHOD"""
    today = datetime.now()
    pdf_path = os.path.join(download_dir, f"AlJarida_Media_{today.strftime('%Y-%m-%d')}.pdf")
    
    if os.path.exists(pdf_path):
        print(f"Using existing PDF file: {pdf_path}")
        return pdf_path
    
    print("Downloading Al-Jarida newspaper...")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("Looking for the newspaper thumbnail...")
            
            # Find newspaper thumbnail
            newspaper_icon = await page.evaluate("""() => {
                ...
                if (topLeftImages.length > 0) {
                    topLeftImages.sort((a, b) => {
            ...
            if newspaper_icon:
                print(f"Found newspaper thumbnail, clicking...")
                await page.mouse.click(newspaper_icon['x'], newspaper_icon['y'])
                ...
                # Check for new pages/tabs with PDF content
                pages = context.pages
                if len(pages) > 1:
                    new_page = pages[-1]
                    
                    # Look for PDF in frames or embedded content
                    pdf_url = None
                    for frame in new_page.frames:
                        try:
                            frame_url = frame.url
                            if frame_url.endswith('.pdf') or 'pdf' in frame_url:
                                pdf_url = frame_url
                                break
                        except Exception:
                            pass
                    ...
                    if not pdf_url:
                        try:
                            pdf_url = await new_page.evaluate('''() => {
                                const embeds = document.querySelectorAll('embed[src*=".pdf"], iframe[src*=".pdf"], object[data*=".pdf"]');
                                return embeds.length > 0 ? (embeds[0].src || embeds[0].data) : null;
                            }''')
                        except Exception:
                            pass
                    
                    # Download if PDF URL found
                    if pdf_url:
...
def create_empty_result(bank_code, today, pdf_path=None, error_msg="No content found"):
    ...
    return {
        ...
        'bank_found': False,
        'relevant_pages': [],
        'analysis_text': f"No mentions of {bank_info['english_name']} found in Al-Jarida. {error_msg}",
        'page_screenshots': {},
        'date': today,
...
async def analyze_all_banks(selected_model="gpt-4o"):
    """
    Main multi-bank analysis function combining Al-Jarida download with Vision API
    """
    today = datetime.now()
    
    print(f"Starting Al-Jarida Vision API multi-bank analysis...")
    print(f"Date: {today.strftime('%Y-%m-%d')}")
    print(f"Model: {selected_model}") 
    print(f"Banks to analyze: {', '.join([get_bank_info(bank)['english_name'] for bank in ACTIVE_BANKS])}")
    print("=" * 60)
    
    # Step 1: Download PDF using ORIGINAL Al-Jarida method
    pdf_path = await download_aljarida()
    ...
    # Step 4: Use Vision API to analyze all pages
    ...
    try:
        ...
        # Analyze all pages
        end_page = min(10, total_pages - 1)  # Analyze pages 6-11 (indices 5-10)
        vision_results = await analyzer.analyze_pdf_pages(pdf_path, 5, end_page)
        #vision_results = await analyzer.analyze_pdf_pages(pdf_path, 0, total_pages - 1)
    ...
    print("\n" + "=" * 60)
    print("AL-JARIDA VISION API MULTI-BANK ANALYSIS SUMMARY")
    print("=" * 60)
...
async def main():
    ...
    print("Al-Jarida Vision API Multi-Bank Analysis Started...")
