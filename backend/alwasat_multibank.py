...
# -*- coding: utf-8 -*-
"""
Al-Wasat Multi-Bank Analysis Script with Vision API Integration
Adapted to use Vision API methodology while preserving Al-Wasat download process
"""
...
# Configuration - Keep Al-Wasat download directory structure
BASE_DATA_DIR = os.getenv("DATA_DIR", "/tmp/bank_news_data")
download_dir = Path(BASE_DATA_DIR) / "AlWasat"
...
"""
    ...
    async def analyze_pdf_pages(self, pdf_path, start_page=3, end_page=8):
    ...
    try:
        if os.path.exists(pdf_path):
            ...
            if file_size > 10000:  # At least 10KB
                ...
                doc.close()
                if pages > 0:
                    print(f"PDF validated: {pages} pages, {file_size} bytes")
                    return True
    except Exception as e:
...
async def try_multiple_download_methods(page, pdf_url, pdf_path, context):
    """Try multiple methods to download the PDF - PRESERVE ORIGINAL METHOD"""
    
    # Method 1: Direct requests download with session cookies
    try:
        ...
        headers = {
            ...
            'Referer': 'https://www.alwasat.com.kw/',
...
async def download_alwasat_pdf():
    """Download the latest Al-Wasat PDF - PRESERVE ORIGINAL METHOD"""
    today = datetime.now()
    pdf_path = os.path.join(download_dir, f"temp_Alwasat_{today.strftime('%Y-%m-%d')}.pdf")
    ...
    print("Downloading Al-Wasat newspaper...")
    ...
    async with async_playwright() as p:
        ...
        try:
            main_url = "https://www.alwasat.com.kw/"
            ...
            pdf_links = await page.evaluate("""() => {
                ...
                for (const element of links) {
                    ...
                    if (href.includes('.pdf') || onclick.includes('.pdf') || 
                        text.includes('عدد اليوم') || text.includes('PDF') || 
                        href.includes('newspaper') || href.includes('اليوم')) {
            ...
            for link in pdf_links:
                if link['visible'] and link['href'] and '.pdf' in link['href']:
                    print(f"Trying PDF URL: {link['href']}")
                    pdf_downloaded = await try_multiple_download_methods(page, link['href'], pdf_path, context)
                    if pdf_downloaded:
                        break
    ...
    if os.path.exists(pdf_path) and await validate_pdf(pdf_path):
        print(f"PDF successfully downloaded to temporary location")
        return pdf_path
    else:
...
def create_empty_result(bank_code, today, pdf_path=None, error_msg="No content found"):
    ...
    return {
        ...
        'bank_found': False,
        'relevant_pages': [],
        'analysis_text': f"No mentions of {bank_info['english_name']} found in Al-Wasat. {error_msg}",
        'page_screenshots': {},
        'date': today,
...
async def analyze_all_banks(selected_model="gpt-4o"):
    """
    Main multi-bank analysis function combining Al-Wasat download with Vision API
    """
    today = datetime.now()
    
    print(f"Starting Al-Wasat Vision API multi-bank analysis...")
    print(f"Date: {today.strftime('%Y-%m-%d')}")
    print(f"Model: {selected_model}")
    print(f"Banks to analyze: {', '.join([get_bank_info(bank)['english_name'] for bank in ACTIVE_BANKS])}")
    print("=" * 60)
    
    # Step 1: Download PDF using ORIGINAL Al-Wasat method
    pdf_path = await download_alwasat_pdf()
    ...
    try:
        ...
        end_page = min(8, total_pages - 1)  # Analyze pages 4-9 (indices 3-8)
        vision_results = await analyzer.analyze_pdf_pages(pdf_path, 3, end_page)
    ...
    print("\n" + "=" * 60)
    print("AL-WASAT VISION API MULTI-BANK ANALYSIS SUMMARY")
    print("=" * 60)
...
async def main():
    ...
    print("Al-Wasat Vision API Multi-Bank Analysis Started...")
