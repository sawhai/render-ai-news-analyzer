...
# -*- coding: utf-8 -*-
"""
Al-Qabas Multi-Bank Analysis Script with Vision API Integration
Adapted to use Vision API methodology while preserving Al-Qabas download process
"""
...
# Configuration - Keep Al-Qabas download directory structure
BASE_DATA_DIR = os.getenv("DATA_DIR", "/tmp/bank_news_data")
download_dir = Path(BASE_DATA_DIR) / "AlQabasMedia"
...
"""
    ...
    async def analyze_pdf_pages(self, pdf_path, start_page=5, end_page=12):
        """Main analysis function with comprehensive debugging for pages 6-13 (indices 5-12)"""
        print("VISION API ANALYSIS STARTED")
        print(f"   PDF: {pdf_path}")
        print(f"   Pages: {start_page + 1} to {end_page + 1} (Pages 6-13)")
...
async def download_alqabas_pdf():
    """Download the Al-Qabas newspaper PDF - PRESERVE ORIGINAL METHOD"""
    today = datetime.now()
    pdf_path = os.path.join(download_dir, f"AlQabas_Media_{today.strftime('%Y-%m-%d')}.pdf")
    
    if os.path.exists(pdf_path):
        print(f"Using existing PDF file: {pdf_path}")
        return pdf_path
    
    print("Downloading AlQabas Media newspaper...")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Extract all links with href containing ".pdf"
            ...
            # Find a link that has both download attribute and target="_blank"
            pdf_url = None
            for link in pdf_links:
                if link.get('hasDownloadAttr') and link.get('targetBlank') and 'آخر عدد' in link.get('text', ''):
                    pdf_url = link.get('href')
                    print(f"Found direct PDF download link: {pdf_url}")
                    break
            
            # If we didn't find the ideal link, use the first PDF link
            if not pdf_url and pdf_links:
                pdf_url = pdf_links[0].get('href')
                print(f"Using first PDF link: {pdf_url}")
            
            # Download PDF directly
            if pdf_url:
                print(f"Downloading PDF directly from URL: {pdf_url}")
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'application/pdf' in content_type or pdf_url.endswith('.pdf'):
                            ...
                            with open(pdf_path, 'wb') as f:
                                for chunk in response.iter_content(chunk_size=8192):
                                    f.write(chunk)
                            print(f"Saved file anyway to: {pdf_path}")
                    else:
                        print(f"Failed to download PDF: Status code {response.status_code}")
                except Exception as download_error:
                    print(f"Error downloading PDF: {str(download_error)}")
            else:
                # Try clicking download link as fallback
                print("No direct PDF URL found, trying to click on download link...")
                
                try:
                    download_link = page.get_by_role("link", name="download icon تحميل آخر عدد")
                    if await download_link.count() > 0:
                        print("Found download link by role, clicking...")
                        ...
                        if len(pages) > 1:
                            new_page = pages[-1]
                            print(f"New page opened, URL: {new_page.url}")
                            
                            # If it's a PDF, download it
                            if 'pdf' in new_page.url:
                                pdf_url = new_page.url
                                print(f"PDF URL in new page: {pdf_url}")
                                
                                response = requests.get(pdf_url)
                                with open(pdf_path, 'wb') as f:
                                    f.write(response.content)
                                print(f"Successfully downloaded PDF from new page")
                        else:
                            # Try to access the href directly
                            href = await download_link.get_attribute("href")
                            if href and 'pdf' in href:
                                print(f"Found PDF URL in href: {href}")
                                response = requests.get(href)
                                with open(pdf_path, 'wb') as f:
                                    f.write(response.content)
                                print(f"Successfully downloaded PDF from href")
                except Exception as click_error:
                    print(f"Error clicking download link: {str(click_error)}")
    ...
    if not os.path.exists(pdf_path) or os.path.getsize(pdf_path) < 10000:
        print("Failed to download PDF or file too small")
        return None
    ...
    try:
        ...
        page_count = len(doc)
        doc.close()
        print(f"Verified PDF has {page_count} pages")
        return pdf_path
    except Exception as verify_error:
        print(f"File is not a valid PDF: {str(verify_error)}")
        return None
...
def create_empty_result(bank_code, today, pdf_path=None, error_msg="No content found"):
    ...
    return {
        ...
        'bank_found': False,
        'relevant_pages': [],
        'analysis_text': f"No mentions of {bank_info['english_name']} found in Al-Qabas. {error_msg}",
        'page_screenshots': {},
        'date': today,
...
async def analyze_all_banks(selected_model="gpt-4o"):
    """
    Main multi-bank analysis function combining Al-Qabas download with Vision API
    """
    today = datetime.now()
    
    print(f"Starting Al-Qabas Vision API multi-bank analysis...")
    print(f"Date: {today.strftime('%Y-%m-%d')}")
    print(f"Model: {selected_model}") 
    print(f"Banks to analyze: {', '.join([get_bank_info(bank)['english_name'] for bank in ACTIVE_BANKS])}")
    print("=" * 60)
    
    # Step 1: Download PDF using ORIGINAL Al-Qabas method
    pdf_path = await download_alqabas_pdf()
    ...
    # Step 4: Use Vision API to analyze pages 6-13 (indices 5-12)
    print(f"Starting Vision API analysis of pages 6-13 from {pdf_path}")
    
    vision_results = {}
    try:
        analyzer = VisionBankAnalyzer(openai_api_key,selected_model)
        # Analyze pages 6-13 (indices 5-12)
        end_page = min(12, total_pages - 1)
    ...
    print("\n" + "=" * 60)
    print("AL-QABAS VISION API MULTI-BANK ANALYSIS SUMMARY")
    print("=" * 60)
...
async def main():
    ...
    print("Al-Qabas Vision API Multi-Bank Analysis Started...")
