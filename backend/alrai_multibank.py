...
# -*- coding: utf-8 -*-
"""
Al-Rai Media Multi-Bank Analysis Script with Vision API Integration
Combines working PDF download with Vision API for robust Arabic/English bank content detection
"""
...
# Import the generated bank patterns - COPIED FROM WORKING VERSION
try:
...
"""
    ...
    async def analyze_pdf_pages(self, pdf_path, start_page=6, end_page=10):
        ...
        try:
            for page_idx in range(start_page, end_page + 1):
                ...
                try:
                    ...
                    # POINT 1: Log raw Vision API response
                    ...
                    for bank_finding in page_result.get('banks_found', []):
                        ...
                        # POINT 2: Log bank mapping attempts
...
async def validate_pdf(pdf_path):
    """Validate downloaded PDF - FROM WORKING VERSION"""
    try:
...
async def download_alrai_pdf():
    """Download the latest AlRai PDF - EXACT COPY FROM WORKING VERSION"""
    today = datetime.now()
    pdf_path = os.path.join(download_dir, f"AlRai_Media_{today.strftime('%Y-%m-%d')}.pdf")
    
    if os.path.exists(pdf_path):
        print(f"Using existing PDF file: {pdf_path}")
        return pdf_path
    
    print("Downloading AlRai newspaper...")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("Navigating to PDF viewer page...")
            await page.goto("https://www.alraimedia.com/newspaper/pdf", wait_until="networkidle", timeout=15000)
            await asyncio.sleep(3)
            
            # Click "إضغط هنا" to open the PDF viewer
            print("Looking for 'إضغط هنا' button...")
            click_here_elements = await page.locator("text=إضغط هنا").all()
            
            if click_here_elements:
                print(f"Found {len(click_here_elements)} 'إضغط هنا' elements")
                await click_here_elements[0].click()
                await page.wait_for_load_state("networkidle", timeout=10000)
                await asyncio.sleep(8)
                
                # Check if we're now on a PDF URL directly
                current_url = page.url
                print(f"Current URL after click: {current_url}")
                
                if current_url.endswith('.pdf') or '.pdf?' in current_url:
                    print("Direct PDF URL detected, downloading...")
                    try:
...
def create_empty_result(bank_code, today, pdf_path=None, error_msg="No content found"):
    ...
    return {
        ...
        'bank_found': False,
        'relevant_pages': [],
        'analysis_text': f"No mentions of {bank_info['english_name']} found in Al-Rai Media. {error_msg}",
        'page_screenshots': {},
        'date': today,
...
async def analyze_all_banks(selected_model="gpt-4o"):
    """
    Main multi-bank analysis function combining working PDF download with Vision API
    """
    today = datetime.now()
    
    print(f"Starting Al-Rai Media Vision API multi-bank analysis...")
    print(f"Date: {today.strftime('%Y-%m-%d')}")
    print(f"Model: {selected_model}") 
    print(f"Banks to analyze: {', '.join([get_bank_info(bank)['english_name'] for bank in ACTIVE_BANKS])}")
    print("=" * 60)
    
    # Step 1: Download PDF using WORKING method
    pdf_path = await download_alrai_pdf()
    ...
    try:
        ...
        if total_pages < 11:
            print(f"WARNING: PDF only has {total_pages} pages, but trying to analyze pages 7-11")
        
        # Check if pages 7-11 exist and have content
        for page_idx in range(6, min(11, total_pages)):
    ...
    # Step 4: Use Vision API to analyze pages 7-11 (indices 6-10)
    print(f"Starting Vision API analysis of pages 7-11 from {pdf_path}")
    #print(f"Using model: gpt-4o-mini")
    
    vision_results = {}
    try:
        analyzer = VisionBankAnalyzer(openai_api_key, selected_model)
        vision_results = await analyzer.analyze_pdf_pages(pdf_path, 6, min(11, total_pages-1))
    ...
    print("\n" + "=" * 60)
    print("AL-RAI MEDIA VISION API MULTI-BANK ANALYSIS SUMMARY")
    print("=" * 60)
...
async def main():
    ...
    print("Al-Rai Media Vision API Multi-Bank Analysis Started...")
