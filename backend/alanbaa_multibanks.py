...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 28 22:45:13 2025
...
# -*- coding: utf-8 -*-
"""
Al-Anbaa Multi-Bank Analysis Script with Vision API Integration
Adapted to use Vision API methodology while preserving Al-Anbaa download process
"""
...
# Configuration - Keep Al-Anbaa download directory structure
BASE_DATA_DIR = os.getenv("DATA_DIR", "/tmp/bank_news_data")
download_dir = Path(BASE_DATA_DIR) / "AlAnbaMedia"
...
"""
    ...
    async def analyze_pdf_pages(self, pdf_path, start_page=4, end_page=9):
        """Main analysis function with comprehensive debugging for pages 5-10 (indices 4-9)"""
        print("VISION API ANALYSIS STARTED")
        print(f"   PDF: {pdf_path}")
        print(f"   Pages: {start_page + 1} to {end_page + 1} (Pages 5-10)")
...
def download_pdf_direct(today):
    """Download PDF directly using known URL pattern with multiple date attempts - PRESERVE ORIGINAL METHOD"""
    ...
    for date_attempt in dates_to_try:
        # Format: pdf.alanba.com.kw/pdf/YYYY/MM/DD-MM-YYYY/DD-MM-YYYY.pdf
        ...
        pdf_path = os.path.join(download_dir, f"AlAnba_Media_{date_attempt.strftime('%Y-%m-%d')}.pdf")
        
        print(f"Trying direct PDF download for {date_attempt.strftime('%Y-%m-%d')}: {pdf_url}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return pdf_path, date_attempt
            else:
                print(f"Failed for {date_attempt.strftime('%Y-%m-%d')}: Status code {response.status_code}")
        except Exception as e:
            print(f"Error downloading PDF for {date_attempt.strftime('%Y-%m-%d')}: {str(e)}")
    
    return None, None

async def download_alanbaa_pdf():
    """Download the latest Al-Anbaa PDF - PRESERVE ORIGINAL METHOD"""
    today = datetime.now()
    
    # Try direct download first
    pdf_path, actual_date = download_pdf_direct(today)
    
    if not pdf_path:
        print("All direct download attempts failed. Trying browser method...")
        
        # If direct download fails, try with browser
        async with async_playwright() as p:
            ...
            try:
                ...
                pdf_elements = await page.evaluate("""() => {
                    ...
                    for (const el of elements) {
                        ...
                        if (text.includes('PDF') || text.includes('pdf') || 
                            href.includes('.pdf') || href.includes('pdf.alanba')) {
                ...
                # Try downloading from likely PDF elements
                for elem in pdf_elements:
                    if elem['href'] and '.pdf' in elem['href']:
                        print(f"Trying to download from: {elem['href']}")
                        try:
                            response = requests.get(elem['href'], stream=True, timeout=30)
                            if response.status_code == 200:
                                pdf_path = os.path.join(download_dir, f"AlAnba_Media_{today.strftime('%Y-%m-%d')}.pdf")
                                with open(pdf_path, 'wb') as f:
                                    for chunk in response.iter_content(chunk_size=8192):
                                        f.write(chunk)
                                print(f"Successfully downloaded PDF: {os.path.getsize(pdf_path)} bytes")
                            ...
                            print(f"Error downloading from {elem['href']}: {str(e)}")
                            
            except Exception as e:
                print(f"Browser method failed: {str(e)}")
    ...
    if pdf_path and os.path.exists(pdf_path):
        try:
            ...
            print(f"Downloaded file is not a valid PDF: {str(verify_error)}")
            return None
...
def create_empty_result(bank_code, today, pdf_path=None, error_msg="No content found"):
    ...
    return {
        ...
        'bank_found': False,
        'relevant_pages': [],
        'analysis_text': f"No mentions of {bank_info['english_name']} found in Al-Anbaa. {error_msg}",
        'page_screenshots': {},
        'date': today,
...
async def analyze_all_banks(selected_model="gpt-4o"):
    """
    Main multi-bank analysis function combining Al-Anbaa download with Vision API
    """
    today = datetime.now()
    
    print(f"Starting Al-Anbaa Vision API multi-bank analysis...")
    print(f"Date: {today.strftime('%Y-%m-%d')}")
    print(f"Model: {selected_model}") 
    print(f"Banks to analyze: {', '.join([get_bank_info(bank)['english_name'] for bank in ACTIVE_BANKS])}")
    print("=" * 60)
    
    # Step 1: Download PDF using ORIGINAL Al-Anbaa method
    pdf_path = await download_alanbaa_pdf()
    ...
    try:
        ...
        # Analyze pages 5-10 (indices 4-9)
        end_page = min(9, total_pages - 1)
        vision_results = await analyzer.analyze_pdf_pages(pdf_path, 4, end_page)
    ...
    print("\n" + "=" * 60)
    print("AL-ANBAA VISION API MULTI-BANK ANALYSIS SUMMARY")
    print("=" * 60)
...
async def main():
    ...
    print("Al-Anbaa Vision API Multi-Bank Analysis Started...")
